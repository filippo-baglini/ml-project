...
num_units = [8, 12, 20]  # Possible number of units for hidden layers
num_layers = [1, 2]
act_funs = [Logistic, Tanh, ReLU, Leaky_ReLU]  # Hidden layer activation functions
learning_rates = [Learning_rate(0.00004), Learning_rate(0.00005), Linear_decay_learning_rate(0.00005, 0.00002, 300), Learning_rate(0.00003)]
regularization = [None, "Tikhonov"]
lambda_values = [None, 0.0001, 0.001, 0.01]
momentum_values = [None, Momentum(0.9), Nesterov_momentum(0.9)]
early_stopping = [Early_stopping(100, 0.0001)]
num_epochs = [1000]

nn, best_eval_loss = grid_search_hold_out(train_data_in, train_data_out, eval_data_in, eval_data_out, num_units, num_layers, act_funs, learning_rates, regularization, lambda_values, momentum_values, early_stopping, num_epochs, task = "Regression")
nn.reset()

retrain_data_in = np.concatenate((train_data_in, eval_data_in))
retrain_data_out = np.concatenate((train_data_out, eval_data_out))

nn.train(retrain_data_in, retrain_data_out, 1000, plot = True)
